...
function init() {
  const program = new commander.Command(packageJson.name)
    ...
    .on('--help', () => {
      ...
      console.log(
        `      ${chalk.cyan(
          'https://github.com/facebook/sugoi-cli/issues/new'
        )}`
      );
      console.log();
    })
  ...
  checkForLatestVersion()
    .catch(() => {
      try {
        return execSync('npm view sugoi-cli version').toString().trim();
      } catch (e) {
        return null;
      }
    })
...
// https://github.com/facebook/sugoi-cli/pull/368#issuecomment-243446094
...
function checkForLatestVersion() {
  return new Promise((resolve, reject) => {
    https
      .get(
        'https://registry.npmjs.org/-/package/sugoi-cli/dist-tags',
        res => {
